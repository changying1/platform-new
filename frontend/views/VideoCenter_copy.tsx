...
export default function VideoCenter() {
  ...
  const ezopenRecoveringRef = useRef<string | null>(null);
  ...
  const handleShowStream = async (device: Video) => {
    try {
      ...
      const info = await getVideoStreamUrl(device.id, {
        forceRefresh: isEzvizCloudDevice(device),
      });
      // 进入全屏时切断全部小屏链路，避免多路会话抢占。
      setPreviewStreams({});
      setPreviewLoading({});
      setPreviewErrors({});
      ezopenRecoveringRef.current = null;
      setStreamInfo(info);
                  ...
                  <div className="flex-1 flex items-center justify-center bg-black relative min-h-0">
                    <div className="relative w-full h-full">
                      <VideoPlayer
                        ...
                        onError={async () => {
                          if (!maximizedVideo) return;
                          const isEzopenStream =
                            String(streamInfo?.play_type || "").toLowerCase() === "ezopen" ||
                            String(streamUrl || "").toLowerCase().startsWith("ezopen://") ||
                            isEzvizCloudDevice(maximizedVideo);
                          if (!isEzopenStream) return;

                          const recoverKey = `${maximizedVideo.id}:${streamUrl}`;
                          if (ezopenRecoveringRef.current === recoverKey) {
                            return;
                          }
                          ezopenRecoveringRef.current = recoverKey;

                          try {
                            clearVideoStreamUrlCache(maximizedVideo.id);
                            const fresh = await getVideoStreamUrl(maximizedVideo.id, { forceRefresh: true });
                            setStreamInfo(fresh);
                            setStreamUrl(fresh.url);
                          } catch (refreshErr) {
                            console.error("EZOPEN 播放失败后刷新流地址仍失败:", refreshErr);
                          }
                        }}
                      />
                      <canvas
