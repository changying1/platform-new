...
def save_playback(video_id: int, body: PlaybackSaveRequest, db: Session = Depends(get_db)):
    """按自定义时间段保存回放视频片段"""
    # 直接按视频ID检查，避免无效ID触发无意义的FFmpeg调用
    video = db.query(VideoDevice).filter(VideoDevice.id == video_id).first()
    if not video:
        raise HTTPException(status_code=404, detail="Video device not found")

    try:
        result = service.save_playback_clip(video_id, body.start_time, body.end_time)
